...
public class DC_Islands : DensityGenerator
{

    [Header("Core")]
    public float coreRadius = 15;

    [Header("Shell")]
    public float shellWidth = 15;
    ...
    public override ComputeBuffer Generate(ComputeBuffer pointsBuffer, int numPointsPerAxis, float boundsSize, Vector3 worldBounds, Vector3 centre, Vector3 offset, float spacing)
    {
        ...
        densityShader.SetFloat("contrast", contrast);

        //Core Buffer
        densityShader.SetFloat("coreRadius", coreRadius);

        //Shell Buffer
        densityShader.SetFloat("shellWidth", shellWidth);
